    ...
    methods
        ...
        function obj = AddToPortfolio(obj,Symbols,Quantity,Cost,MarketData,Direction,StrategyData)
            ...
            for i = 1:size(Symbols,2)
                if ~isempty(obj.Symbols) 
                   ...
                   %/ Otherwise calculate weighted average of position
                       ...
                       if obj.Quantity(1,Index)*obj.Direction(1,Index) == - Quantity(1,i) * Direction(1,i) %/ close position
                          obj.Cost(1,Index) = 0;
           ...
           if isempty(obj.Quantity)
              obj.Quantity =  Quantity .* Direction; 
           else
              obj.Quantity =  obj.Quantity .* obj.Direction + Quantity .* Direction;  
           end    
           %/ Trade Direction
        ...
        %/ Remove security to the portfolio object
        function obj = RemoveFromPortfolio(obj,Symbols,Quantity,Cost,MarketData,Direction,StrategyData)
           %/ input check
           if size(Symbols,1) > 1 || ~iscell(Symbols)
              error('Symbols must be a 1 x n cell');
           end
           ...
           %/ update current position
            ...
            %/ add quantity 
            obj.Quantity(1,Index) =  obj.Quantity(1,Index)*obj.Direction(1,Index) + Quantity(i)* Direction(1,i); 
           
            %/ Trade Direction
            obj.Direction = obj.Direction + Direction; 
            
            %/ NAV Calculation(it is remove function hence no security left in the portfolio at this stage)
             obj.NAV = obj.Cash;
             obj.NAVHistory(1,end+1) = obj.NAV; 
